...
public class FuelQueue {
    ...
    static  Passenger[] waitingList = new Passenger[6];         //declare the waiting list
    ...
    static int front = -1;      //declare the front for circular queue
    static int rear = -1;       //declare the rear for circular queue
    ...
    public void enQueueValeu(Passenger newPerson){
        if (front==-1 && rear ==-1){
            front = 0;
            rear = 0;
    ...
    public void addCustomer(){          //add newOne to queue
        if (queueLength()!=6){
            ...
            if (!((rear+1)%6==front)){
                ...
                income = (int) (income + (430*liters));         //calculate income
                Passenger newOne = new Passenger(firstname, secondName, vehicleNumber, (int) liters);
                enQueueValeu(newOne);
                fuelStock=fuelStock-liters;
                count+=1;
...
//                deQValue();
    ...
    public static void viewRemainingFuelStock(){
        System.out.println("Remaining Fuel Stock is "+fuelStock);       //remaining fuel stock
    }
